...
export default function SelectedWork() {
  const picks = ["Eagle Pathway", "Commercial Bank of Ethiopia", "Taptosign", "EagleBot"];

  const items = projects.pinProjects.filter((p) => picks.includes(p.title));
  const displayItems = items.length > 0 ? items : projects.pinProjects.slice(0, 4);
    ...
    <section className="mt-8 scroll-mt-14" id="selected-work">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
        {displayItems.map((item, idx) => (
          <article
            key={idx}
            className="p-4 border-2 border-base-content/10 rounded-xl hover:border-base-content/80 transition flex flex-col justify-between"
          >
            <div>
                ...
                <div className="flex items-center gap-2">
                  {item.caseStudy && (
                    <Link
                      ...
                      aria-label={`Read case study for ${item.title}`}
                      className="text-xs border border-base-content/20 hover:border-base-content/80 rounded px-2 py-0.5 text-base-content/70 hover:text-base-content transition"
                    >
                      Case study
                    </Link>
                  )}
